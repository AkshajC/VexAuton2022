...
void autonomous(void) {
  ...
  resetDriveSensors = true;   // reset encoders to 0
  //intakeSpin(100);
  desiredValue = 900;     // desired turn distance = 370 degrees. Make negative to turn other direction
...
//DRIVE BACK TO HOME ZONE
  ...
  resetDriveSensors = true;   // reset encoders to 0
  //intakeSpin(100);
  desiredValue = -555;     // desired turn distance = 370 degrees. Make negative to turn other direction
  ...
  // ringIntake.startSpinTo(-20, degrees);



//turn left TOWARDS home goal
  backMogoControlAuton();
  x = 1;            //X contols turning; use 1 when turning, and 0 when going straight
  z = 1;            //Z controls direction; use 0 when using positive integers, and 1 when using negative ones.
  turnspeed = -40;
  resetDriveSensors = true;   // reset encoders to 0
  //intakeSpin(100);
  desiredTurnValue = -430;     // desired turn distance = 370 degrees. Make negative to turn other direction
  vex::task::sleep(1500);    // the drivePID task will run for only 2ooo ms.
  leftBackMotor.spin(directionType:: rev, speed, velocityUnits::pct);
  leftMiddleMotor.spin(directionType:: fwd, speed, velocityUnits::pct);
  leftFrontMotor.spin(directionType:: fwd, speed, velocityUnits::pct);
  ...
  // liftMotor.startSpinTo(2000, degrees);
...
//drive back TO GRAB HOME GOAL
  ...
  resetDriveSensors = true;   // reset encoders to 0
  //intakeSpin(100);
  desiredValue = -250;     // desired turn distance = 370 degrees. Make negative to turn other direction
  vex::task::sleep(1500);    // the drivePID task will run for only 2ooo ms.
  leftBackMotor.spin(directionType:: rev, speed, velocityUnits::pct);
  leftMiddleMotor.spin(directionType:: fwd, speed, velocityUnits::pct);
  leftFrontMotor.spin(directionType:: fwd, speed, velocityUnits::pct);
